    ...
    'format_theme': [
        {
            ...
            'format_instructions': parser_theme.get_format_instructions(),
    ...
    'format_generation': [
        {
            ...
            'format_instructions': parser_generation.get_format_instructions(),
    ...
    'format_distractors': [
        {
            ...
            'format_instructions': parser_distractors.get_format_instructions(),
...
def get_model(primary_model, few_shot=False):
    llm = Ollama(model=primary_model)
    ...
    theme_chain = theme_prompt_template | llm
    ...
    format_theme_prompt_few_shot  = ChatPromptTemplate.from_messages(
        [
            ("human", "Вопрос: {reference_question}\nТщательно проанализируй пример и используй цепочку размышлений и напиши свой анализ вопроса и общей темы. Тема не должна быть слишком специфичной, а более общей. После этого, опиши общую тему вопроса."),
            ("ai", "{response}"),
            ("human", "Подведи итог анализа темы вопроса и отформатируй сгенерированный ответ (тему) на русском языке в формате JSON. Ты должен строго следовать инструкциям и вывести только JSON и ничего больше. Инструкции для формата:\n{format_instructions}"),
            ("ai", "{output}")
        ]
    )
        ...
        generation_prompt_template = ChatPromptTemplate.from_messages(
            [
                *([generation_few_shot_prompt] if few_shot else []),
                ...
                ("human", "# Тема вопроса: {theme}\n\n# Пример вопроса по теме:\n```{reference_question}```\n\n# Существующие вопросы:\n1. ```{reference_question}```" + "\n".join([f"{i+2}. ```{question.to_string()}```" for i, question in enumerate(existing_questions)]) + "\n\n\nТы должен сгенерировать вопрос и правильный ответ к нему по данной теме для студенческого экзамена. Не генерируй вопросы похожие на существующие!. Для начала тщательно проанализируй тему и используй цепочку размышлений и напиши свой анализ. После этого, сгенерируй вопрос и правильный ответ по этой общей теме. Ты не должен повторять или перефразировать существующие вопросы. Ты должен сгенерировать уникальный вопрос и один правильный вариант ответа, без других вариантов. Не генерируй варианты ответа, только правильный ответ. Не добавляй букву или номер к ответу. Ты должен генерировать уникальные вопросы и ответы, достаточно сильно отличающиеся от существующих вопросов, но на ту же общую тему."),
            ]
        )

        return generation_prompt_template | llm
    
    format_generation_few_shot  = ChatPromptTemplate.from_messages(
        [
            ("human", "Отформатируй сгенерированный вопрос и правильный ответ к нему в формате JSON. Ты дожен взять именно сгенерированный вопрос и ответ, а не вопрос из приимера.\n{format_instructions}\nТекст: {response}\n"),
            ("ai", "{output}")
        ]
    )
    ...
    def generate_theme(reference_question):
        while True:
                try:
                    theme_response = theme_chain.invoke({"reference_question": reference_question})
                    log_debug("theme_response", theme_response)
                    formatted_theme = (format_theme_prompt | llm2).invoke({"reference_question": reference_question, "response": theme_response, "format_instructions": parser_theme.get_format_instructions()})
    ...
    def generate_exam_question(theme, reference_question, existing_questions: List[ExamQuestion]):
        ...
        while attempts <= 6:
            while True:
                try:
                    ...
                    generated_response = generation_chain.invoke({"theme": theme.question_theme, "reference_question": reference_question})
                    log_debug("generated_response", generated_response)
                    formatted_generation = (format_generation_prompt | llm2).invoke({"response": generated_response, "format_instructions": parser_generation.get_format_instructions()})
                    log_debug("formatted_generation", formatted_generation)
                    generated_question = parser_generation.invoke(formatted_generation)
            ...
            validation_response = validation_chain.invoke({"generated_question": generated_question.question, "correct_answer": generated_question.correct_answer})
            log_debug("validation_response", validation_response)

            # Step 3: Summarize the validation answer
            summary_response = summary_chain.invoke({"generated_question": generated_question.question, "correct_answer": generated_question.correct_answer, "validation_answer": validation_response})
        ...
        while True:
            try:
                ...
                distractors_response = distractors_chain.invoke({"theme": theme.question_theme, "generated_question": generated_question.question, "correct_answer": generated_question.correct_answer})
                log_debug("distractors_response", distractors_response)
                formatted_distractors = (format_distractors_prompt | llm2).invoke({"response": distractors_response, "format_instructions": parser_distractors.get_format_instructions()})
        ...
        exam_question = ExamQuestion(
            ...
            distractors=[distractors.distractor_1, distractors.distractor_2, distractors.distractor_3]
        )
...
if __name__ == "__main__":
    # Example usage
    print('Distractors')
    print(parser_distractors.get_format_instructions())
    print()
    print('Theme')
    print(parser_theme.get_format_instructions())
    print()
    print('Generation')
    print(parser_generation.get_format_instructions())
    print()
    """ reference_question = "Какой элемент имеет атомный номер 6?\nA) Гелий\nB) Кислород\nC) Углерод\nD) Водород"
    generate_exam_question = get_model('llama3')
    exam_question = generate_exam_question(reference_question)

    print(f"Вопрос: {exam_question.question}")
    print(f"Правильный ответ: {exam_question.correct_answer}")
    print(f"Дистракторы: {', '.join(exam_question.distractors)}") """
