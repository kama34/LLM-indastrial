...
from baseline import get_model, get_model_list, ExamQuestion
...
generate_theme, generate_exam_question = get_model(model_name, few_shot_learning)
print(f'Loaded {model_name} model with few-shot={few_shot_learning}')
...
if template_question is not None:
    selected_question_index = int(template_question.split(':')[0].split()[1]) - 1
    ...
    examples = random.choices(parsed_questions[:selected_question_index] + parsed_questions[selected_question_index+1:], k=5)
...
num_questions = st.number_input("Количество вопросов для генерации", min_value=1, max_value=10, value=1)
is_rendered = False
...
if 'generate_button' in st.session_state and st.session_state.generate_button:
        ...
        st.markdown(f"<div style='padding: 5px; background-color: #e2f0d9; color: #155724; border: 1px solid #c3e6cb; border-radius: 5px; margin-bottom: 12px;'>{theme.question_theme}</div>", unsafe_allow_html=True)
        ...
        if 'theme_confirmed' in st.session_state and st.session_state.theme_confirmed:
            ...
            for i in range(num_questions):
                with st.spinner(f"Генерация вопроса {i + 1} из {num_questions}..."):
                    try:
                        exam_question = generate_exam_question(theme, reference_question, questions)
                        ...
                        with st.expander(f"Сгенерированный вопрос {i + 1}", expanded=(i == 0)):
                            ...
                            st.markdown(f"<div style='padding: 5px; background-color: #e2f0d9; color: #155724; border: 1px solid #c3e6cb; border-radius: 5px; margin-bottom: 12px;'>{exam_question.correct_answer}</div>", unsafe_allow_html=True)
...
if "generated_questions" in st.session_state:
    ...
    for exam_question in questions:
        all_answers = [exam_question.correct_answer] + exam_question.distractors
